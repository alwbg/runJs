...
(function (global) {
	...
	merge(ModuleWorker.prototype, {
		...
		load: function (args) {
			...
			var max = 15 * 1000;
			(function run() {
				if (Tag.sheet || times > max) return tools.runFx.call(self, args.callback, args.data);
				times += step;
				setTimeout(run, step);
			}())
		},
	...
	function moduleFactory(id) {
		...
		if (task = com.isInStorage(id)) {
			...
			try {
				...
				tools.runFx(require.error || define.error || tools.config.info, e);
				// echo('%c创建异常...' + e, 'color:red' )
			}
			if (isFunction(_ret)) {
				_Mo.exports = _ret;
			} else {
